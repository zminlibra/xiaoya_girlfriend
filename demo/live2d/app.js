...
(() => {
  ...
  // CustomVoice 固定音色（Qwen3-TTS-12Hz-1.7B-CustomVoice）
  //
  // id 即模型内置的 speaker 名，音色由预训练说话人嵌入决定，**不会随生成漂移**。
  // 注意：不要改回 VoiceDesign 的文字描述式音色——VoiceDesign 每次都会重新"设计"
  // 一个新音色，同一个描述连续生成也会得到不同声线（实测基频漂移约 17%），
  // 无法维持稳定的"人格声音"。稳定音色是选 CustomVoice 的唯一理由。
  // 说话风格（口语化/停顿/气声）改由后端 qwen3_tts_instruct 统一控制。
  const QWEN_VOICES = [
    { id: "serena", label: "女声 · 温柔清澈 (Serena) ★推荐" },
    { id: "vivian", label: "女声 · 甜美活泼 (Vivian)" },
    { id: "sohee", label: "女声 · 轻柔细腻 (Sohee)" },
    { id: "ono_anna", label: "女声 · 元气日系 (Ono Anna)" },
    { id: "aiden", label: "男声 · 低沉磁性 (Aiden)" },
    { id: "dylan", label: "男声 · 温暖阳光 (Dylan)" },
    { id: "eric", label: "男声 · 清朗爽快 (Eric)" },
    { id: "ryan", label: "男声 · 阳光少年 (Ryan)" },
    { id: "uncle_fu", label: "男声 · 憨厚朴实 (Uncle Fu)" },
  ];
